...
for year in ${years[@]}
	...
	if [ $1 = "UL2016" ]
		...
		then
		./run/make_variations_loop_newntuple.exe nominal on 500 1400 ${year} 109
		./run/make_variations_loop_newntuple.exe kappa on 500 1400 ${year} 109
	else
	...
	if [ ${mconly} = "on" ]
		...
		if [ $1 = "UL2016" ]
			...
			then
			echo "./run/rescale_variations_kappa.exe mconly variations/output_nominal_newnt_${year}.root"
		...
		if [ $1 = "UL2016" ]
			...
			then
			rm variations/output_CRFit_${year}.root
			hadd -f variations/output_CRFit_${year}.root variations/output_nominal_newnt_nl0shape_${year}.root variations/output_kappa_newnt_nl0shape_${year}.root
		else
			rm variations/output_CRFit_${year}.root
			...
			#./run/make_jer_symm.exe ${year} variations/output_CRFit_${year}.root
			cp variations/output_nominal_newnt_${year}.root variations/output_CRFit_${year}.root
		fi
	fi
	cp variations/output_CRFit_${year}.root datacards/variations/output_CRFit_${year}.root
	if [ $1 = "UL2016" ]
		then
		./run/make_rpv_datacard_newbins_CRfit.exe 1800 mconly output_CRFit_${year}.root ${year} UL2016 off
		...
		then
		./run/make_rpv_datacard_newbins_CRfit.exe 1800 mconly output_CRFit_${year}.root ${year} UL20178 off
		combineCards.py datacards/datacard_M1800_mconly_cr_nb0_*_CRFit_${year}.dat datacards/datacard_M1800_mconly_cr_nb1_*_CRFit_${year}.dat > datacards/datacard_M1800_CRFit_${year}.dat
	elif [ $1 = "2024" ]
		then
		./run/make_rpv_datacard_newbins_2024_CRfit.exe 1800 mconly output_CRFit_${year}.root ${year} 2024 off
		combineCards.py datacards/datacard_M1800_mconly_cr_nb0_*_CRFit_${year}.dat datacards/datacard_M1800_mconly_cr_nb1_*_CRFit_${year}.dat > datacards/datacard_M1800_CRFit_${year}.dat
	else
...
if [ $1 = "UL2016" ]
	...
	then
	combine -M FitDiagnostics datacards/datacard_M1800_CRFit_2024.dat --saveNorm --saveShape --cminDefaultMinimizerType Minuit
	root -l -q repack.C\(\"CRFit_2024\"\)
fi
