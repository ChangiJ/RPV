...
//      kappa_unc[i-1][ind_ibin][ind_proc] = TMath::Sqrt(unc_diff*unc_diff+unc_stat*unc_stat+unc_mc*unc_mc+unc_dy*unc_dy)/temp_mc_;  //FIXME
  ...
  hist_kappa1->SetMaximum(3.0);
  hist_kappa1->SetMinimum(0.0001);
  hist_kappa1->SetTitle("");
  hist_kappa2->SetStats(0);
  hist_kappa2->SetMaximum(3.0);
  hist_kappa2->SetMinimum(0.0001);
...
// CG 260320 temporalily removed
  ...
  hist_kappa1->GetYaxis()->SetTitle("");
  hist_kappa1->GetYaxis()->SetLabelSize(0.06);
  hist_kappa1->GetYaxis()->ChangeLabel(-1, -1, 0);
  hist_kappa1->GetXaxis()->SetLabelSize(0);

  hist_kappa2->GetYaxis()->SetTitle("");
  hist_kappa2->GetYaxis()->SetLabelSize(0.06);
  hist_kappa2->GetYaxis()->ChangeLabel(-1, -1, 0);

  for (int i = 0; i < 3; i++) {
      hist_kappa2->GetXaxis()->SetBinLabel(i*3 + 1, "#splitline{Low}{(4-5)}");
      hist_kappa2->GetXaxis()->SetBinLabel(i*3 + 2, "#splitline{Mid}{(6-7)}");
      hist_kappa2->GetXaxis()->SetBinLabel(i*3 + 3, "#splitline{High}{(#geq8)}");
  }
  hist_kappa2->GetXaxis()->SetLabelSize(0.08);
  hist_kappa2->GetXaxis()->SetLabelOffset(0.015);
  hist_kappa2->GetXaxis()->SetTickLength(0.05);

  hist_kappa1->SetMarkerStyle(20);
  hist_kappa1->SetMarkerSize(1.5);
  hist_kappa1->SetMarkerColor(kBlack);
  hist_kappa1->SetLineColor(kBlack);
  hist_kappa1->SetLineWidth(2);

  hist_kappa2->SetMarkerStyle(20);
  hist_kappa2->SetMarkerSize(1.5);
  hist_kappa2->SetMarkerColor(kBlack);
  hist_kappa2->SetLineColor(kBlack);
  hist_kappa2->SetLineWidth(2);

  int col_qcd   = TColor::GetColor("#ffcc00");
  int col_wjets = TColor::GetColor("#9933cc");
  int col_ttbar = TColor::GetColor("#3366ff");

  pad1->cd();
  hist_kappa1->Draw("ex0");

  TBox *b1 = new TBox(0., 0.0001, 3., 3.0);
  b1->SetFillColor(col_qcd);
  TBox *b2 = new TBox(3., 0.0001, 6., 3.0);
  b2->SetFillColor(col_wjets);
  TBox *b3 = new TBox(6., 0.0001, 9., 3.0);
  b3->SetFillColor(col_ttbar);
  ...
  latex->DrawLatex(0.12, 0.90, "CMS #font[52]{Preliminary}");
  latex->SetTextAlign(31);
  latex->DrawLatex(0.95, 0.90, Form("%.1f fb^{-1} (13 TeV)", lumi));

  TLatex *texY1 = new TLatex();
  texY1->SetNDC();
  texY1->SetTextFont(42);
  texY1->SetTextSize(0.10);
  texY1->SetTextAlign(22);
  texY1->DrawLatex(0.06, 0.88, "#kappa_{1}");
  pad1->RedrawAxis();

  pad2->cd();
  hist_kappa2->Draw("ex0");

  TBox *b4 = new TBox(0., 0.0001, 3., 3.0);
  b4->SetFillColor(col_qcd);
  TBox *b5 = new TBox(3., 0.0001, 6., 3.0);
  b5->SetFillColor(col_wjets);
  TBox *b6 = new TBox(6., 0.0001, 9., 3.0);
  b6->SetFillColor(col_ttbar);
  ...
  TLatex *texProc = new TLatex();
  texProc->SetNDC();
  texProc->SetTextFont(42);
  texProc->SetTextSize(0.09);
  texProc->SetTextAlign(22); // Center alignment
  texProc->DrawLatex(0.258, 0.04, "QCD");
  texProc->DrawLatex(0.535, 0.04, "W+jets");
  texProc->DrawLatex(0.811, 0.04, "t#bar{t}");

  TLatex *texY2 = new TLatex();
  texY2->SetNDC();
  texY2->SetTextFont(42);
  texY2->SetTextSize(0.10);
  texY2->SetTextAlign(22);
  texY2->DrawLatex(0.06, 0.95, "#kappa_{2}");
