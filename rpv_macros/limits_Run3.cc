...
void drawHeader()
{
  ...
  lat->SetTextFont(42);
//  lat->DrawLatexNDC(0.66, 0.93, "36.3 fb^{-1} (13 TeV)");//FIXME
  lat->DrawLatexNDC(0.66, 0.93, "300 fb^{-1} (13.6 TeV)");//FIXME
}

void pulls()
{
  std::vector<float> gluinoMasses = {1500, 1600, 1700, 1800, 1900, 2000, 2100, 2200, 2300, 2400, 2500};
...
void limits_new()
{
  ...
  for(unsigned int i=0; i<gluinoMasses.size(); i++) {
    ...
    TString filename(Form("newbin_250919/higgsCombine_M%d.AsymptoticLimits.mH120.root", static_cast<int>(gluinoMasses.at(i))));
  ...
  drawHeader();
  c->Print("limits_"+year+"_250919.pdf");
  c->Print("limits_"+year+"_250919.png");
}
