...
/*
	//============================================================	
	// Define ranges
	std::vector<std::string> njetRanges = {"4 < N_{jet} < 5", "6 < N_{jet} < 7", "8 < N_{jet}"};
	std::vector<std::string> mjRanges = {"500 < M_{J} < 800 GeV", "800 < M_{J} < 1100 GeV", "1100 GeV < M_{J}"};
	std::vector<std::string> nbRanges = {"0", "1", "2", "3", ">4"};

	const int nRows = nbRanges.size();
	const int nCols = njetRanges.size() * mjRanges.size();
	const int totalPlots = nRows * nCols; // Total number of pie charts
	int currentPlot = 0; // Track progress

	// Create the canvas
	TCanvas* canvas = new TCanvas("canvas", "Table with Pie Charts", 2500, 2000);
	canvas->Divide(nCols + 1, nRows + 1);


	// Add row titles (N_b ranges)
	for (int i = 0; i < nRows; ++i) {
					TPaveText* nbTitle = new TPaveText(0.05, 1.0 - (i + 2) * 0.98 / (1+nRows), 0.13, 1.0 - (i+1) * 0.98 / (1+nRows), "NB");
					nbTitle->AddText(("N_b = " + nbRanges[i]).c_str());
					nbTitle->SetFillColor(0);
					nbTitle->Draw();
	}

	// Add column titles (N_jet ranges)
	for (int j = 0; j < nCols/3; ++j) {
					TPaveText* colTitle = new TPaveText(0.14 + 0.26 * j, 0.85, 0.44 + 0.26 * j, 0.88, "NB");
					colTitle->AddText(njetRanges[j].c_str());
					colTitle->SetFillColor(0);
					colTitle->Draw();
	}
	

	// Fill the table with pie charts
	for (int i = 0; i < nRows; ++i) {
					for (int j = 0; j < nCols; ++j) {
									int njetIndex = j / mjRanges.size();
									int mjIndex = j % mjRanges.size();
									int binNum = 22 + i * 3 + njetIndex;  // Adjust bin number logic
									if (binNum == 34 || binNum == 49) continue; // Exclude specific bins

									// Adjust chart positions (ensure valid range)
									double x1 = 0.15 + j * 0.8 / nCols;
									double x2 = 0.15 + (j + 1) * 0.8 / nCols; 
									double y1 = 0.83 - (i + 1) * 0.8 / nRows;
									double y2 = 0.83 - i * 0.8 / nRows;

									// Check if positions are within [0, 1]
									if (x1 >= 1.0 || x2 > 1.0 || y1 >= 1.0 || y2 > 1.0) {
													std::cerr << "Error: Invalid TPad positions for bin " << binNum << std::endl;
													continue;
									}

									// Create TPad
									TPad* pad = new TPad(Form("pad_%d_%d", i, j), "", x1, y1, x2, y2);
									canvas->cd();
									pad->SetFillColor(0);
									...
									TPad* pad = (TPad*)canvas->cd((i + 1) * (nCols+1) + (j+1));
									pad->SetFillColor(0);
									pad->SetFrameFillColor(0);
									

									pie4[binNum][mjIndex]->Draw();
					}
	}
	*/
	// Save as PDF
	//canvas->SaveAs(outputdir+Form("piechart_result.pdf"));
	//canvas->SaveAs(outputdir+Form("piechart_result.png"));
	//std::cout << "\nTable generation completed successfully!" << std::endl;
}
